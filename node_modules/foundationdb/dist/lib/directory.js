...
exports.DirectoryLayer = exports.Directory = exports.HighContentionAllocator = exports.counterEncoding = exports.DirectoryError = void 0;
const _1 = require(".");
...
exports.counterEncoding = {
    pack(val) {
        const b = Buffer.alloc(8);
        if (!Number.isSafeInteger(val))
            throw new DirectoryError('Invalid counter (number outside JS safe range)');
...
class HighContentionAllocator {
    constructor(subspace) {
        ...
        this.counters = subspace.withKeyEncoding(_1.tuple).at(0).withValueEncoding(exports.counterEncoding);
...
class Directory {
    constructor(parentDirectoryLayer, path, contentSubspace, isPartition, layer) {
        ...
        if (isPartition) {
            ...
            this._layer = layer ? util_1.asBuf(layer) : null;
    ...
    list(txn, path = []) {
        return __asyncGenerator(this, arguments, function* list_1() {
            yield __await(yield* __asyncDelegator(__asyncValues(this._directoryLayer.list(txn, this._partitionSubpath(path)))));
        });
    }
    ...
    _partitionSubpath(path, directoryLayer = this._directoryLayer) {
        ...
        return (_a = this._path) === null || _a === void 0 ? void 0 : _a.slice(directoryLayer._path.length).concat(normalize_path(path));
    }
...
class DirectoryLayer {
    constructor(opts = {}) {
        ...
        // an invalid tuple.
        var _a, _b;
        this._nodeSubspace = ((_a = opts.nodeSubspace) === null || _a === void 0 ? void 0 : _a.withKeyEncoding(_1.tuple)) || new subspace_1.default(opts.nodePrefix == null ? DEFAULT_NODE_PREFIX : opts.nodePrefix, _1.tuple, transformer_1.defaultTransformer);
        this._contentSubspace = ((_b = opts.contentSubspace) === null || _b === void 0 ? void 0 : _b.withKeyEncoding(_1.tuple)) || new subspace_1.default(opts.contentPrefix == null ? BUF_EMPTY : opts.contentPrefix, _1.tuple, transformer_1.defaultTransformer);
    ...
    async _createOrOpenInternal(txnOrDb, _path, layer = BUF_EMPTY, prefix, allowCreate = true, allowOpen = true) {
        const path = normalize_path(_path);
        // For layers, an empty string is treated the same as a missing layer property.
        const layerBuf = util_1.asBuf(layer);
        ...
        return doTxn(txnOrDb, async (txn) => {
            ...
            if (existing_node.exists()) {
                ...
                if (existing_node.isInPartition()) {
                    ...
                    return await existing_node.getContentsSync(this)._directoryLayer._createOrOpenInternal(txn, subpath, layer, prefix, allowCreate, allowOpen);
                }
                else {
                    if (!allowOpen)
                ...
                if (prefix == null) {
                    ...
                    prefix = util_1.concat2(this._contentSubspace.prefix, await this._allocator.allocate(txn));
                    if ((await txn.at(subspace_1.root).getRangeAllStartsWith(prefix, { limit: 1 })).length > 0) {
                        throw new DirectoryError('The database has keys stored at the prefix chosen by the automatic prefix allocator: ' + util_2.inspect(prefix));
                    }
    ...
    list(txn, _path) {
        return __asyncGenerator(this, arguments, function* list_2() {
            var e_1, _a;
            yield __await(this._checkVersion(txn, false));
            const path = normalize_path(_path);
            const node = yield __await(this.findWithMeta(txn, path));
            if (!node.exists())
                throw new DirectoryError('The directory does not exist.');
            if (node.isInPartition(true)) {
                yield __await(yield* __asyncDelegator(__asyncValues(node.getContentsSync(this).list(txn, node.getPartitionSubpath()))));
            }
            else {
                try {
                    for (var _b = __asyncValues(this._subdirNamesAndNodes(txn, node.subspace)), _c; _c = yield __await(_b.next()), !_c.done;) {
                        const [name] = _c.value;
    ...
    listAll(txnOrDb, _path = []) {
        return doTxn(txnOrDb, async (txn) => {
            ...
            const results = [];
            try {
                for (var _b = __asyncValues(this.list(txn, _path)), _c; _c = await _b.next(), !_c.done;) {
                    const path = _c.value;
                    results.push(path);
                }
            }
    ...
    async _nodeContainingKey(txn, key) {
        ...
        if (util_1.startsWith(key, this._nodeSubspace.prefix))
        ...
        if (prev) {
            ...
            if (util_1.startsWith(key, prev_prefix))
    ...
    contentSubspaceForNode(node) {
        return new subspace_1.default(this.getPrefixForNode(node), transformer_1.defaultTransformer, transformer_1.defaultTransformer);
    }
    async find(txn, path) {
    ...
    _contentsOfNode(nodeSubspace, path, layer = BUF_EMPTY) {
        // This is some black magic. We have a reference to the node's subspace, but
        // what we really want is the prefix. So we want to do the inverse to
        // this._nodeSubspace.at(...).
    ...
    _subdirNamesAndNodes(txn, node) {
        return __asyncGenerator(this, arguments, function* _subdirNamesAndNodes_1() {
            var e_3, _a;
            try {
                // TODO: This could work using async iterators to improve performance of searches on very large directories.
                for (var _b = __asyncValues(txn.at(node).getRange(SUBDIRS_KEY)), _c; _c = yield __await(_b.next()), !_c.done;) {
                    const [key, prefix] = _c.value;
                    yield yield __await([key[1], this._nodeWithPrefix(prefix)]);
                }
            }
    ...
    async _subdirNamesAndNodesAll(txn, node) {
        ...
        const items = [];
        try {
            for (var _b = __asyncValues(this._subdirNamesAndNodes(txn, node)), _c; _c = await _b.next(), !_c.done;) {
                const pair = _c.value;
                items.push(pair);
            }
        }
    ...
    async _isPrefixFree(txn, prefix) {
        ...
        return prefix.length > 0
            ...
            && (await txn.at(this._nodeSubspace).getRangeAll(prefix, util_1.strInc(prefix), { limit: 1 })).length === 0;
    }
}
