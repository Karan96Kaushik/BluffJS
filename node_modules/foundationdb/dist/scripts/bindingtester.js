...
const tupleStrict = Object.assign(Object.assign({}, lib_1.tuple), { name: 'tuple strict', unpack: val => lib_1.tuple.unpack(val, true) });
...
const makeMachine = (db, initialName) => {
    ...
    const popSelector = async () => {
        ...
        return lib_1.keySelector(key, orEqual, Number(offset));
    };
    ...
    const pushTupleItem = (data) => {
        pushValue(data);
        if (verbose)
            console.log(chalk.green('(encodes to)'), lib_1.tuple.pack(data));
    };
    ...
    const operations = {
        ...
        async TUPLE_PACK_WITH_VERSIONSTAMP() {
            ...
            try {
                ...
                const pack = versionstamp_1.packPrefixedVersionstamp(prefix, value, true);
        ...
        async DIRECTORY_LOG_SUBSPACE(oper) {
            const prefix = await popBuffer();
            await oper.set(util_1.concat2(prefix, lib_1.tuple.pack(dirIdx)), getCurrentSubspace().prefix);
        },
        ...
        async DIRECTORY_STRIP_PREFIX() {
            ...
            if (!util_1.startsWith(byteArray, prefix)) {
