...
util_1.withEachDb(db => describe('key value functionality', () => {
    ...
    it('retries conflicts', async function () {
        ...
        const key = 'num';
        await db.set(key, util_1.numToBuf(0));
        let txnAttempts = 0;
        await Promise.all(new Array(concurrentWrites).fill(0).map((_, i) => (db.doTransaction(async (tn) => {
            const val = util_1.bufToNum((await tn.get(key)));
            tn.set(key, util_1.numToBuf(val + 1));
            txnAttempts++;
        }))));
        const result = util_1.bufToNum((await db.get(key)));
    ...
    describe('getKey', () => {
        it('returns the exact key requested', async () => {
            var _a;
            await db.set('x', 'y');
            assert.strictEqual((_a = (await db.getKey('x'))) === null || _a === void 0 ? void 0 : _a.toString(), 'x');
        });
        ...
        it('returns the key that matches if a key selector is passed', async () => {
            ...
            await db.set('a', 'y');
            await db.set('b', 'y');
            assert.strictEqual((_a = (await db.getKey(lib_1.keySelector.firstGreaterThan('a')))) === null || _a === void 0 ? void 0 : _a.toString(), 'b');
        });
        it('returns undefined if the key selector matches outside of the subspace range', async () => {
            await db.at('b').set('key', 'val');
            const _db = db.at('a');
            assert.strictEqual(await _db.getKey(lib_1.keySelector('', true, 0)), undefined);
        });
    });
    ...
    describe('regression', () => {
        it('does not trim off the end of a string', async () => {
            ...
            assert.strictEqual(val === null || val === void 0 ? void 0 : val.toString(), 'val');
        });
    });
}));
